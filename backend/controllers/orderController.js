...
export async function createOrder(req, res) {
  ...
  try {
    ...
    if (!userId)
      return res
        .status(401)
        .json({ message: "Unauthorized. Invalid user information in token." });
    ...
    if (lastOrder) {
      ...
      orderData.orderId = "ORD" + (lastNumber + 1).toString().padStart(6, "0");
    }

    // ← use cart data directly, no DB lookup
    for (const item of req.body.items) {
      orderData.items.push({
        product: null,
        name: item.name,
    ...
    orderData.subtotal = orderData.total; // ← set subtotal
...
export async function getOrders(req, res) {
  ...
  try {
    const filter = isAdmin(req) ? {} : { email: req.user.email };
