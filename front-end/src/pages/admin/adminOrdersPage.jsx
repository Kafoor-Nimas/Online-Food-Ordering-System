	...
	return (
		<div className="w-full h-full overflow-y-auto relative bg-gray-50">
			<div className="flex items-center justify-between gap-3 px-8 pt-8 pb-5">
				<div>
                    <p className="text-xs font-semibold tracking-[0.2em] text-orange-500 uppercase mb-1">Sales</p>
                ...
                {!loading && (
                    <div className="hidden sm:flex items-center gap-2 bg-white border border-gray-200 rounded-full px-4 py-2 shadow-sm">
                        ...
                        <span className="text-sm text-gray-600">{orders.length} on this page</span>
                    </div>
                )}
			</div>
			...
			{ loading ? (
                ...
                <div className="w-full py-24 flex flex-col justify-center items-center text-center">
                    ...
                    <p className="text-sm text-gray-600 mt-1">Orders will show up here once customers start buying.</p>
                </div>
            ) : (
				...
				<table className="min-w-[1100px] w-full text-sm relative">
                    <thead>
						<tr className="bg-orange-500 text-white">
							...
							<th className="px-5 py-4 text-center text-xs font-semibold uppercase tracking-wider whitespace-nowrap">Actions</th>
						</tr>
					</thead>
                    <tbody className="text-gray-700 divide-y divide-gray-200">
                        {orders.map((order) => (
                            <tr key={order._id || order.id} className="odd:bg-gray-50 hover:bg-orange-50 transition-colors">
                                <td className="px-5 py-4 whitespace-nowrap font-medium text-gray-800">{order.orderId || order._id}</td>
                                <td className="px-5 py-4 whitespace-nowrap">{(order.firstName || '') + " " + (order.lastName || '')}</td>
                                <td className="px-5 py-4 whitespace-nowrap text-gray-600">{order.email || '-'}</td>
                                <td className="px-5 py-4 whitespace-nowrap text-gray-600">{new Date(order.date).toLocaleDateString() || '-'}</td>
                                <td className="px-5 py-4 whitespace-nowrap font-semibold text-gray-800">${order.total?.toFixed(2) || '0.00'}</td>
                                <td className="px-5 py-4 whitespace-nowrap">
                                    <span className={`inline-block px-2.5 py-1 text-xs font-semibold rounded-full ${statusStyles[order.status] || "bg-gray-200 text-gray-700"}`}>
            ...
            <div className="w-full absolute bottom-6 left-0 h-[56px] flex justify-center items-center px-4">
                <div className="w-full max-w-[520px] h-full bg-white shadow-xl shadow-gray-400/10 border border-gray-200 rounded-full flex items-center justify-center px-2 gap-1">
                    <button className="bg-orange-500 w-[100px] text-white text-sm font-semibold py-2 rounded-full cursor-pointer hover:bg-orange-600 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                    ...
                    <button className="bg-orange-500 text-white text-sm font-semibold py-2 rounded-full w-[100px] cursor-pointer hover:bg-orange-600 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
